CREATE FUNCTION SearchAnnoucements(
    ...
    @p_engine_powers INT = NULL,
    @p_engine_volume DECIMAL(2, 1) = NULL,
...
BEGIN
    ...
    WHERE
        ...
        (@p_run IS NULL OR C.run <= @p_run) AND
		...
		(@p_engine_powers IS NULL OR C.engine_powers = @p_engine_powers) AND
		(@p_engine_volume IS NULL OR C.engine_volume = @p_engine_volume) AND
